...
class vibe_gray:
    ...
    def Background_initialize(self, image):
    ...
    def Foreground_Classify(self, image):
    ...
    def Background_Update(self, image, updating_mask):
        numberOfTests = self.numberOfSamples - self.numberOfHistoryImages
    ...
    def apply(self, frame):
        ...
        if(self.height == 0 or self.width == 0):
            self.Background_initialize(frame)
            # print('init: %.4f' % (time.time() - t1))
        # t2 = time.time()
        mask = self.Foreground_Classify(frame)
        # t3 = time.time()
        self.Background_Update(frame, mask)
        # t4 = time.time()
        #median blur
        ...
        # update_time += (t4 - t3)
        # print('segmentation: %.4f, update: %.4f' % (segmentation_time, update_time)) 
